...
def test_temperature_defaults_to_zero(monkeypatch) -> None:
    """Principle 14: scores compared across runs cannot carry sampling noise."""
    monkeypatch.setenv("OPENAI_API_KEY", "test-key")
    assert get_llm("openai").temperature == 0.0


def test_no_model_name_hardcoded_in_agents() -> None:
    """The rubric fails a hardcoded provider; the factory is the only place a
    model string may appear."""
