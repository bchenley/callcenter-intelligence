...
# Switching providers is an env-var change, never a code change. Model names
# live here so no agent module hardcodes one.
...
def get_llm(
    ...
    runs, so sampling noise is a defect here, not a feature. Imports are deferred
    per provider so a missing optional SDK breaks only the provider that needs it."""
