...
BILLING_RESOLVED: list[Line] = [
    Line(
        "Agent",
        "Thank you for calling Northwind Utilities, this is Marcus. How can I help you today?",
    ),
    Line(
        "Customer",
        "Hi, yes. My account was charged twice this month and I need help getting one of them back.",
    ),
    Line(
        "Agent",
...
def line_key(line: Line) -> str:
    ...
    script edit re-renders only the lines that actually changed."""
...
def render_samples(client, n_jobs: int = 1, only: str | None = None) -> list[tuple[Path, int]]:
    OUT_DIR.mkdir(parents=True, exist_ok=True)
    targets = {only: CALLS[only]} if only else CALLS
    rendered: list[tuple[Path, int]] = []
    for name, lines in targets.items():
        out = build(name, lines, client, n_jobs)
        rendered.append((out, len(lines)))
    return rendered
...
def main() -> int:
    ...
    if args.only is not None and args.only not in CALLS:
        print(
            f"unknown call {args.only!r}; expected one of {', '.join(CALLS)}",
            file=sys.stderr,
        )
        return 1

    client = OpenAI(api_key=cfg.openai_api_key)
    for out, n_lines in render_samples(client, args.n_jobs, args.only):
        size_kb = out.stat().st_size // 1024
        print(f"{out}  ({n_lines} lines, {size_kb} KB)")
    return 0
