...
newer majors. This exercises exactly the API surface the milestones require, so a breaking
...
def _langgraph():
    ...
    g.add_conditional_edges("start_node", router, {"good_node": "good_node", "bad_node": "bad_node"})
...
@check("Gradio: Textbox(show_copy_button=True) [spec requires this in Milestone 6]")
...
# ---------------------------------------------------------------- whisper
@check("faster-whisper: transcribe() accepts the spec's settings (no model download)")
def _whisper():
    import inspect
...
# ---------------------------------------------------------------- LLM providers
@check("LLM factory: all three provider classes import and construct [rubric 20%]")
def _providers():
...
if __name__ == "__main__":
    ...
    if failed:
        ...
        sys.exit(1)
    print("Stack matches the spec's API surface. Safe to build on these versions.")
